...
public final class TelaEdd_Venda_LojaJardim extends JFrame implements ActionListener, Config_idiomas {
    ...
    public TelaEdd_Venda_LojaJardim() throws SQLException {
    ...
    private void configListarDesignacao() {
        ...
        try {
            ...
            try {
                ...
                this.conexao.close();
                if(Idiomas.getPort() == true) {
                    JOptionPane.showMessageDialog(null, "Erro no configListarDesignacao() na TelaEdd_Venda_LojaJardim \n" + erro, "Atenção", 0);
                } else {
                    JOptionPane.showMessageDialog(null, "Error in configListarDesignacao() at TelaEdd_Venda_LojaJardim \n" + erro, "Attention", 0);
                }
            } catch (SQLException ex) {
                Logger.getLogger(TelaEdd_Venda_LojaJardim.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
    
    private void configListarDesignacao2() {
        String sql = "select * from tbloja_jardim where MARCA = ? and CATEGORIA = ? and DESIGNAÇÃO = ?";
    ...
    private void configListarMarca() {
        ...
        try {
            ...
            try {
                ...
                this.conexao.close();
                if(Idiomas.getPort() == true) {
                    JOptionPane.showMessageDialog(null, "Erro no configListarMarca() TelaEdd_Venda_LojaJardim \n" + erro, "Atenção", 0);
                } else {
                    JOptionPane.showMessageDialog(null, "Error in TelaEdd_Venda_LojaJardim configListarMarca() \n" + erro, "Attention", 0);
                }
            } catch (SQLException ex) {
                Logger.getLogger(TelaEdd_Venda_LojaJardim.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void txtCategoria() {
        ...
        try {
            ...
            try {
                ...
                this.conexao.close();
                if(Idiomas.getPort() == true) {
                    JOptionPane.showMessageDialog(null, "Erro no txtCategoria() TelaEdd_Venda_LojaJardim \n" + erro, "Atenção", 0);
                } else {
                    JOptionPane.showMessageDialog(null, "Error in TelaEdd_Venda_LojaJardim txtCategoria() \n" + erro, "Attention", 0);
                }
            } catch (SQLException ex) {
                Logger.getLogger(TelaEdd_Venda_LojaJardim.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void update() throws SQLException {
        String sql = "update tbvenda_jardim set designação = ?, marca = ?, categoria = ?, unidade = ?, quantidade = ?, preco_artigo_mzn = ?, preco_artigo_usd = ?, preco_venda = ? where id = ?";
        try {
            pst = conexao.prepareStatement(sql);
            ...
            if (adicionado > 0) {
                ...
                this.conexao.close();
            if(Idiomas.getPort() == true) {
                JOptionPane.showMessageDialog(null, erro+" MENSAGEM DE ERRO AO TENTAR SALVAR UMA VENDA na TelaEdd_Venda_LojaJardim !", "AVISO", 0);
            } else {
                JOptionPane.showMessageDialog(null, erro+" ERROR MESSAGE WHEN TRYING TO SAVE A SALLING IN TelaEdd_Venda_LojaJardim!", "WARNING", 0);
            }
        }       
    }
