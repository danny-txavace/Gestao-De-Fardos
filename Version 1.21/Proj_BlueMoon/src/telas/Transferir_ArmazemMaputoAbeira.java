...
public final class Transferir_ArmazemMaputoAbeira extends JFrame implements ActionListener, Config_idiomas {
    ...
    public Transferir_ArmazemMaputoAbeira() throws SQLException {
    ...
    private void Janela() {        
        if(Idiomas.getPort() == true) {
            setTitle("TRANSFERIR PARA ARMAZÉM DA BEIRA");
        } else {
            setTitle("TRANSFER TO WAREHOUSE BEIRA");
        }
        setSize(1000, 540);        
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                try {
                    ...
                    this.rs.close();
                    this.conexao.close();        
                    JOptionPane.showMessageDialog(null, "Erro no configListarDesignacao() - Transferir_ArmazemMaputobeira \n" + erro, "Atenção", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                try {
                    this.pst.close();
                    this.rs.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null, "Error in configListarDesignacao() - Transferir_ArmazemMaputobeira \n" + erro, "Attention", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                try {
                    ...
                    this.rs.close();
                    this.conexao.close();        
                    JOptionPane.showMessageDialog(null, "Erro no configListarMarca() - Transferir_ArmazemMaputoAbeira \n" + erro, "Atenção", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                try {
                    ...
                    this.rs.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null, "Error in configListarMarca() - Transferir_ArmazemMaputoAbeira \n" + erro, "Attention", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            ...
            txtCategoria = new JComboBox<>(comboBoxModel);
                ...
                if(Idiomas.getPort() == true) {
                    try {
                        ...
                        this.rs.close();
                        this.conexao.close();        
                        JOptionPane.showMessageDialog(null, "Erro no txtCategoria() - Transferir_ArmazemMaputobeira \n" + erro, "Atenção", 0);
                    } catch (SQLException ex) {
                        Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                    }
                } else {
                    try {
                        ...
                        this.rs.close();
                        this.conexao.close();
                        JOptionPane.showMessageDialog(null, "Error in txtCategoria()- Transferir_ArmazemMaputobeira \n" + erro, "Attention", 0);
                    } catch (SQLException ex) {
                        Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                    }
            }
    ...
    private void temp_loja_Beira() throws SQLException {
        Statement ctemp = conexao.createStatement();
        ctemp.executeUpdate("create temporary table temp_tbarmazem_beira like tbarmazem_beira");         
    }
    
    private void tabelaDAO() {
        String sql = "SELECT ID, DESIGNAÇÃO, MARCA, CATEGORIA, QUANTIDADE FROM temp_tbarmazem_beira";
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                try {
                    ...
                    this.rs.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null,er + " ERRO NA TABELA_DAO() - Transferir_ArmazemMaputo", "TBL   DAO", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                try {
                    ...
                    this.rs.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null,er + " ERROR IN DAO_TABLE() - Transferir_ArmazemMaputo", "TBL   DAO", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            }            
    ...
    private void salvar() throws SQLException {
        String sql = "insert into temp_tbarmazem_beira(designação, marca, categoria, unidade, quantidade, preco_artigo_mzn, preco_artigo_usd) values (?, ?, ?, ?, ?, ?, ?)";
        try {
            pst = conexao.prepareStatement(sql);
            ...
            if(Idiomas.getPort() == true) {
                this.pst.close();
                this.conexao.close();
                JOptionPane.showMessageDialog(null, erro+" MENSAGEM DE ERRO AO TENTAR SALVAR UMA VENDA - Transferir_ArmazemMaputo!", "AVISO", 0);
            } else {
                this.pst.close();
                this.conexao.close();
                JOptionPane.showMessageDialog(null, erro+" ERROR MESSAGE WHEN TRYING TO SAVE - Transferir_ArmazemMaputo!", "WARNING", 0);
            }
        }
        
        String sql2 = "select sum( QUANTIDADE ) as somatemparmazem_beira from temp_tbarmazem_beira";
        int somaVlr = 0;
        try{
            rs = conexao.createStatement().executeQuery(sql2);
            
            while (rs.next()) {
                somaVlr = rs.getInt("somatemparmazem_beira");
            }          
            ...
            if(Idiomas.getPort() == true) {
                ...
                this.conexao.close();
                this.pst.close();
                JOptionPane.showMessageDialog(null, "ERRO NO save() somatemploja_armazem_beira - Transferir_ArmazemMaputo ..! "+err, "Atenção", 0);
            } else {
                this.rs.close();
                this.conexao.close();
                this.pst.close();
                JOptionPane.showMessageDialog(null, "ERROR IN save() somatemploja_armazem_beira - Transferir_ArmazemMaputo ..! "+err, "Attention", 0);
            }            
        }        
    }
    ...
    private void configTransferir1() throws SQLException {
        String sqlTemp = "select DESIGNAÇÃO, QUANTIDADE from temp_tbarmazem_beira";
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                ...
                this.rs.close();
                this.conexao.close();
                JOptionPane.showMessageDialog(null, erro+" MENSAGEM DE ERRO no configUTransferir1() - Transferir_ArmazemMaputo - Update ArmazemMaputo!", "AVISO", 0);
            } else {
                this.pst.close();
                this.rs.close();
                this.conexao.close();
                JOptionPane.showMessageDialog(null, erro+" ERROR MESSAGE in configTransferir1() - Transferir_ArmazemMaputo - Update ArmazemMaputo!", "WARNING", 0);
            }
        }
    }
    
    private void configTransferirBeira() throws SQLException {
        String sqlTemp = "select DESIGNAÇÃO, QUANTIDADE from temp_tbarmazem_beira";
        
        pst = conexao.prepareStatement(sqlTemp);
        rs = pst.executeQuery();
        
        String sqlUpdate_Loja2 = "update tbarmazem_beira set QUANTIDADE = QUANTIDADE + ? WHERE DESIGNAÇÃO = ?";
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                ...
                this.rs.close();
                this.conexao.close();
                JOptionPane.showMessageDialog(null, erro+" MENSAGEM DE ERRO no configUTransferirBeira() - Transferir_ArmazemMaputo - Update Armazem da Beira!", "AVISO", 0);
            } else {
                this.pst.close();
                this.rs.close();
                this.conexao.close();
                JOptionPane.showMessageDialog(null, erro+" ERROR MESSAGE in configTransferirBeira() - Transferir_ArmazemMaputo - Update Beira Warehouse!", "WARNING", 0);
            }
        }
    }
    
    private void limparTabela() {
        String sql = "drop temporary table temp_tbarmazem_beira";
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                try {
                    this.pst.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null, erro+" MENSAGEM DE ERRO no limparTabela() - Transferir_ArmazemMaputo!", "AVISO", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                try {
                    this.pst.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null, erro+" ERROR MESSAGE in limparTabela() - Transferir_ArmazemMaputo!", "WARNING", 0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public void remover() {
        String sql = "delete from temp_tbarmazem_beira where ID = ?";
        ...
        try {
            ...
            if(Idiomas.getPort() == true) {
                try {
                    this.pst.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null," ERRO no remover() - Transferir_Loja2beira...! "+Er, "Atenção",0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                try {
                    this.pst.close();
                    this.conexao.close();
                    JOptionPane.showMessageDialog(null,"ERROR WHILE TRYING TO DELETE in remove() - Transferir_Loja2beira...! "+Er, "Attention",0);
                } catch (SQLException ex) {
                    Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public void actionPerformed(ActionEvent evt) {
        if(evt.getSource() == btnAdicionar) {
            ...
            try {
                configTransferir1();
                configTransferirBeira();
                if(Idiomas.getPort() == true) {
                    JOptionPane.showMessageDialog(null, "Transferido com sucesso para ARMAZÉM DA BEIRA!", "Atenção", 1);
                } else {
                    JOptionPane.showMessageDialog(null, "Successfully Transferred to BEIRA WAREHOUSE!", "Successfully", 1);
                }
            } catch (SQLException ex) {
                Logger.getLogger(Transferir_ArmazemMaputoAbeira.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public void configPort() {
        ...
        txtuser.setText("Armazém da Beira");
    }
    ...
    public void configEng() {
        ...
        txtuser.setText("Beira Warehouse");
    }
}